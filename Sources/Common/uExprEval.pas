...
{**************************************************************************************************}
{                                                                                                  }
{ Last modified: $Date:: 2011-07-12 21:52:46 +0200 (mar., 12 juil. 2011)                         $ }
{ Revision:      $Rev:: 99                                                                       $ }
...
type
  TFloat = TSensorValue; //JclBase.Float;
  PFloat = ^TFloat;
  ...
  T4Func = function(X, Y, Z, Z1: TFloat): TFloat;

  T0FuncOfObject = function(): TFloat of object;
  T1FuncOfObject = function(X1: TFloat): TFloat of object;
  T2FuncOfObject = function(X1, X2: TFloat): TFloat of object;
  T3FuncOfObject = function(X1, X2, X3: TFloat): TFloat of object;
  T4FuncOfObject = function(X1, X2, X3, X4: TFloat): TFloat of object;
...
type
  ...
  public
    ...
    function Call4Func(AFunc: T4Func; X, Y, Z, Z1: TExprNode): TExprNode; virtual; abstract;

    function Call0FuncOfObject(AFunc: T0FuncOfObject): TExprNode; virtual; abstract;
    function Call1FuncOfObject(AFunc: T1FuncOfObject; X1: TExprNode): TExprNode; virtual; abstract;
    function Call2FuncOfObject(AFunc: T2FuncOfObject; X1, X2: TExprNode): TExprNode; virtual; abstract;
    function Call3FuncOfObject(AFunc: T3FuncOfObject; X1, X2, X3: TExprNode): TExprNode; virtual; abstract;
    function Call4FuncOfObject(AFunc: T4FuncOfObject; X1, X2, X3, X4: TExprNode): TExprNode; virtual; abstract;
...
{ some concrete class descendants follow... }
  ...
  public
    ...
    function Call4Func(AFunc: T4Func; X, Y, Z, Z1: TExprNode): TExprNode;  override;

    function Call0FuncOfObject(AFunc: T0FuncOfObject): TExprNode; override;
    function Call1FuncOfObject(AFunc: T1FuncOfObject; X1: TExprNode): TExprNode; override;
    function Call2FuncOfObject(AFunc: T2FuncOfObject; X1, X2: TExprNode): TExprNode; override;
    function Call3FuncOfObject(AFunc: T3FuncOfObject; X1, X2, X3: TExprNode): TExprNode; override;
    function Call4FuncOfObject(AFunc: T4FuncOfObject; X1, X2, X3, X4: TExprNode): TExprNode; override;
  ...
  TExpr4FuncSym = class(TExprAbstractFuncSym)
  private
    FFunc: T4Func;
  public
    constructor Create(const AIdent: string; AFunc: T4Func);
    function Evaluate: TFloat; override;
    function Compile: TExprNode; override;
  end;


  TExpr0FuncSymOfObject = class(TExprAbstractFuncSym)
  private
    FFunc: T0FuncOfObject;
  public
    constructor Create(const AIdent: string; AFunc: T0FuncOfObject);
    function Evaluate: TFloat; override;
    function Compile: TExprNode; override;
  end;

  TExpr1FuncSymOfObject = class(TExprAbstractFuncSym)
  private
    FFunc: T1FuncOfObject;
  public
    constructor Create(const AIdent: string; AFunc: T1FuncOfObject);
    function Evaluate: TFloat; override;
    function Compile: TExprNode; override;
  end;

  TExpr2FuncSymOfObject = class(TExprAbstractFuncSym)
  private
    FFunc: T2FuncOfObject;
  public
    constructor Create(const AIdent: string; AFunc: T2FuncOfObject);
    function Evaluate: TFloat; override;
    function Compile: TExprNode; override;
  end;

  TExpr3FuncSymOfObject = class(TExprAbstractFuncSym)
  private
    FFunc: T3FuncOfObject;
  public
    constructor Create(const AIdent: string; AFunc: T3FuncOfObject);
    function Evaluate: TFloat; override;
    function Compile: TExprNode; override;
  end;

  TExpr4FuncSymOfObject = class(TExprAbstractFuncSym)
  private
    FFunc: T4FuncOfObject;
  public
    constructor Create(const AIdent: string; AFunc: T4FuncOfObject);
    function Evaluate: TFloat; override;
    function Compile: TExprNode; override;
  end;
  ...
  public
    ...
    procedure AddFunc(const AName: string; AFunc: TFloatFunc); overload;
    procedure AddFunc(const AName: string; AFunc: TUnaryFunc); overload;
    procedure AddFunc(const AName: string; AFunc: TBinaryFunc); overload;
    procedure AddFunc(const AName: string; AFunc: TTernaryFunc); overload;
    procedure AddFunc(const AName: string; AFunc: T4Func); overload;

    procedure AddFuncOfObject(const AName: string; AFunc: T0FuncOfObject); overload;
    procedure AddFuncOfObject(const AName: string; AFunc: T1FuncOfObject); overload;
    procedure AddFuncOfObject(const AName: string; AFunc: T2FuncOfObject); overload;
    procedure AddFuncOfObject(const AName: string; AFunc: T3FuncOfObject); overload;
    procedure AddFuncOfObject(const AName: string; AFunc: T4FuncOfObject); overload;
...
//        begin
//          // Alex ++
//          cp^ := FormatSettings.DecimalSeparator;
//          //
        if cp^ = '.' then
        begin
          Inc(cp);
...
type
  ...
  TExprCall4VmOp = class(TExprVirtMachOp)
  private
    FFunc: T4Func;
    FX, FY, FZ, FZ1: PFloat;
  public
    constructor Create(AFunc: T4Func; X, Y, Z, Z1: PFloat);
    procedure Execute; override;
  end;
  ...
  { function of object calls }

  TExprCall0VmOpOfObject = class(TExprVirtMachOp)
  private
    FFunc: T0FuncOfObject;
  public
    constructor Create(AFunc: T0FuncOfObject);
    procedure Execute; override;
  end;

  TExprCall1VmOpOfObject = class(TExprVirtMachOp)
  private
    FFunc: T1FuncOfObject;
    FX: PFloat;
  public
    constructor Create(AFunc: T1FuncOfObject; X: PFloat);
    procedure Execute; override;
  end;

  TExprCall2VmOpOfObject = class(TExprVirtMachOp)
  private
    FFunc: T2FuncOfObject;
    FX1, FX2: PFloat;
  public
    constructor Create(AFunc: T2FuncOfObject; X1, X2: PFloat);
    procedure Execute; override;
  end;

  TExprCall3VmOpOfObject = class(TExprVirtMachOp)
  private
    FFunc: T3FuncOfObject;
    FX1, FX2, FX3: PFloat;
  public
    constructor Create(AFunc: T3FuncOfObject; X1, X2, X3: PFloat);
    procedure Execute; override;
  end;

  TExprCall4VmOpOfObject = class(TExprVirtMachOp)
  private
    FFunc: T4FuncOfObject;
    FX1, FX2, FX3, FX4: PFloat;
  public
    constructor Create(AFunc: T4FuncOfObject; X1, X2, X3, X4: PFloat);
    procedure Execute; override;
  end;
...
type
  ...
  TExprCall4VmNode = class(TExprVirtMachNode)
  private
    FFunc: T4Func;
  public
    constructor Create(AFunc: T4Func; X, Y, Z, Z1: TExprNode);
    procedure GenCode(AVirtMach: TExprVirtMach); override;
  end;

  { of object}
  TExprCall0VmNodeOfObject = class(TExprVirtMachNode)
  private
    FFunc: T0FuncOfObject;
  public
    constructor Create(AFunc: T0FuncOfObject);
  ...
  TExprCall1VmNodeOfObject = class(TExprVirtMachNode)
  private
    FFunc: T1FuncOfObject;
  public
    constructor Create(AFunc: T1FuncOfObject; X: TExprNode);
    procedure GenCode(AVirtMach: TExprVirtMach); override;
  end;

  TExprCall2VmNodeOfObject = class(TExprVirtMachNode)
  private
    FFunc: T2FuncOfObject;
  public
    constructor Create(AFunc: T2FuncOfObject; X1, X2: TExprNode);
    procedure GenCode(AVirtMach: TExprVirtMach); override;
  end;

  TExprCall3VmNodeOfObject = class(TExprVirtMachNode)
  private
    FFunc: T3FuncOfObject;
  public
    constructor Create(AFunc: T3FuncOfObject; X1, X2, X3: TExprNode);
    procedure GenCode(AVirtMach: TExprVirtMach); override;
  end;

  TExprCall4VmNodeOfObject = class(TExprVirtMachNode)
  private
    FFunc: T4FuncOfObject;
  public
    constructor Create(AFunc: T4FuncOfObject; X1, X2, X3, X4: TExprNode);
    procedure GenCode(AVirtMach: TExprVirtMach); override;
  end;
...
function TExprVirtMachNodeFactory.Call4Func(AFunc: T4Func; X, Y, Z, Z1: TExprNode): TExprNode;
begin
  Result := AddNode(TExprCall4VmNode.Create(AFunc, X, Y, Z, Z1));
end;
...
function TExprVirtMachNodeFactory.Call0FuncOfObject(AFunc: T0FuncOfObject): TExprNode;
begin
  Result := AddNode(TExprCall0VmNodeOfObject.Create(AFunc));
end;

function TExprVirtMachNodeFactory.Call1FuncOfObject(AFunc: T1FuncOfObject; X1: TExprNode): TExprNode;
begin
  Result := AddNode(TExprCall1VmNodeOfObject.Create(AFunc, X1));
end;

function TExprVirtMachNodeFactory.Call2FuncOfObject(AFunc: T2FuncOfObject; X1, X2: TExprNode): TExprNode;
begin
  Result := AddNode(TExprCall2VmNodeOfObject.Create(AFunc, X1, X2));
end;

function TExprVirtMachNodeFactory.Call3FuncOfObject(AFunc: T3FuncOfObject; X1, X2, X3: TExprNode): TExprNode;
begin
  Result := AddNode(TExprCall3VmNodeOfObject.Create(AFunc, X1, X2, X3));
end;

function TExprVirtMachNodeFactory.Call4FuncOfObject(AFunc: T4FuncOfObject; X1, X2, X3, X4: TExprNode): TExprNode;
begin
  Result := AddNode(TExprCall4VmNodeOfObject.Create(AFunc, X1, X2, X3, X4));
end;
...
function TExpr4FuncSym.Compile: TExprNode;
var
  X, Y, Z, Z1: TExprNode;
begin
  X := CompileFirstArg;
  ...
  Z1 := CompileNextArg;
  EndArgs;
  Result := NodeFactory.Call4Func(FFunc, X, Y, Z, Z1);
end;

constructor TExpr4FuncSym.Create(const AIdent: string; AFunc: T4Func);
begin
  Assert(Assigned(AFunc));
  inherited Create(AIdent);
  FFunc := AFunc;
...
function TExpr4FuncSym.Evaluate: TFloat;
var
  X, Y, Z, Z1: TFloat;
begin
  X := EvalFirstArg;
  ...
  Z1 := EvalNextArg;
  EndArgs;
  Result := FFunc(X, Y, Z, Z1);
end;

procedure TEasyEvaluator.AddFuncOfObject(const AName: string; AFunc: T0FuncOfObject);
begin
  FOwnContext.Add(TExpr0FuncSymOfObject.Create(AName, AFunc));
end;

procedure TEasyEvaluator.AddFuncOfObject(const AName: string; AFunc: T1FuncOfObject);
begin
  FOwnContext.Add(TExpr1FuncSymOfObject.Create(AName, AFunc));
end;

procedure TEasyEvaluator.AddFuncOfObject(const AName: string; AFunc: T2FuncOfObject);
begin
  FOwnContext.Add(TExpr2FuncSymOfObject.Create(AName, AFunc));
end;

procedure TEasyEvaluator.AddFuncOfObject(const AName: string; AFunc: T3FuncOfObject);
begin
  FOwnContext.Add(TExpr3FuncSymOfObject.Create(AName, AFunc));
end;

procedure TEasyEvaluator.AddFunc(const AName: string; AFunc: TUnaryFunc);
begin
  FOwnContext.Add(TExprUnaryFuncSym.Create(AName, AFunc));
end;

procedure TEasyEvaluator.AddFunc(const AName: string; AFunc: TFloatFunc);
begin
  FOwnContext.Add(TExprFuncSym.Create(AName, AFunc));
end;

procedure TEasyEvaluator.AddFunc(const AName: string; AFunc: TBinaryFunc);
begin
  FOwnContext.Add(TExprBinaryFuncSym.Create(AName, AFunc));
end;

procedure TEasyEvaluator.AddFunc(const AName: string; AFunc: T4Func);
begin
  FOwnContext.Add(TExpr4FuncSym.Create(AName, AFunc));
end;

procedure TEasyEvaluator.AddFunc(const AName: string; AFunc: TTernaryFunc);
begin
  FOwnContext.Add(TExprTernaryFuncSym.Create(AName, AFunc));
end;

procedure TEasyEvaluator.AddFuncOfObject(const AName: string; AFunc: T4FuncOfObject);
begin
  FOwnContext.Add(TExpr4FuncSymOfObject.Create(AName, AFunc));
end;
...
constructor TExprCall4VmNode.Create(AFunc: T4Func; X, Y, Z, Z1: TExprNode);
begin
  FFunc := AFunc;
  inherited Create([X, Y, Z, Z1]);
end;

procedure TExprCall4VmNode.GenCode(AVirtMach: TExprVirtMach);
begin
  FExprVmCode := TExprCall4VmOp.Create(
    FFunc,
    VmDeps[0].ExprVmCode.OutputLoc,
    VmDeps[1].ExprVmCode.OutputLoc,
    VmDeps[2].ExprVmCode.OutputLoc,
    VmDeps[3].ExprVmCode.OutputLoc);
  AVirtMach.Add(FExprVmCode);
end;
...
constructor TExprCall4VmOp.Create(AFunc: T4Func; X, Y, Z, Z1: PFloat);
begin
  inherited Create;
  FFunc := AFunc;
  FX := X;
...
procedure TExprCall4VmOp.Execute;
begin
  FOutput := FFunc(FX^, FY^, FZ^, FZ1^);
end;
...
{ TExprCall1VmNodeOfObject }

constructor TExprCall1VmNodeOfObject.Create(AFunc: T1FuncOfObject; X: TExprNode);
begin
  FFunc := AFunc;
  inherited Create([X]);
end;

procedure TExprCall1VmNodeOfObject.GenCode(AVirtMach: TExprVirtMach);
begin
  FExprVmCode := TExprCall1VmOpOfObject.Create(
    FFunc,
    VmDeps[0].ExprVmCode.OutputLoc);
  AVirtMach.Add(FExprVmCode);
end;

{ TExprCall1VmOpOfObject }

constructor TExprCall1VmOpOfObject.Create(AFunc: T1FuncOfObject; X: PFloat);
begin
  inherited Create;
  FFunc := AFunc;
  FX := X;
...
procedure TExprCall1VmOpOfObject.Execute;
begin
  FOutput := FFunc(FX^);
end;

{ TExprCall2VmOpOfObject }

constructor TExprCall2VmOpOfObject.Create(AFunc: T2FuncOfObject; X1, X2: PFloat);
begin
  inherited Create;
  FFunc := AFunc;
  FX1 := X1;
...
procedure TExprCall2VmOpOfObject.Execute;
begin
  FOutput := FFunc(FX1^, FX2^);
end;

{ TExprCall3VmOpOfObject }

constructor TExprCall3VmOpOfObject.Create(AFunc: T3FuncOfObject; X1, X2, X3: PFloat);
begin
  inherited Create;
  FFunc := AFunc;
  FX1 := X1;
...
procedure TExprCall3VmOpOfObject.Execute;
begin
  FOutput := FFunc(FX1^, FX2^, FX3^);
end;

{ TExprCall4VmOpOfObject }

constructor TExprCall4VmOpOfObject.Create(AFunc: T4FuncOfObject; X1, X2, X3, X4: PFloat);
begin
  inherited Create;
  FFunc := AFunc;
  FX1 := X1;
...
procedure TExprCall4VmOpOfObject.Execute;
begin
  FOutput := FFunc(FX1^, FX2^, FX3^, FX4^);
end;

{ TExprCall2VmNodeOfObject }

constructor TExprCall2VmNodeOfObject.Create(AFunc: T2FuncOfObject; X1, X2: TExprNode);
begin
  FFunc := AFunc;
  inherited Create([X1, X2]);
end;

procedure TExprCall2VmNodeOfObject.GenCode(AVirtMach: TExprVirtMach);
begin
  FExprVmCode := TExprCall2VmOpOfObject.Create(
    FFunc,
    VmDeps[0].ExprVmCode.OutputLoc,
    VmDeps[1].ExprVmCode.OutputLoc);
  AVirtMach.Add(FExprVmCode);
...
{ TExprCall3VmNodeOfObject }

constructor TExprCall3VmNodeOfObject.Create(AFunc: T3FuncOfObject; X1, X2, X3: TExprNode);
begin
  FFunc := AFunc;
  inherited Create([X1, X2, X3]);
end;

procedure TExprCall3VmNodeOfObject.GenCode(AVirtMach: TExprVirtMach);
begin
  FExprVmCode := TExprCall3VmOpOfObject.Create(
    FFunc,
    VmDeps[0].ExprVmCode.OutputLoc,
    VmDeps[1].ExprVmCode.OutputLoc,
    VmDeps[2].ExprVmCode.OutputLoc
...
{ TExprCall4VmNodeOfObject }

constructor TExprCall4VmNodeOfObject.Create(AFunc: T4FuncOfObject; X1, X2, X3, X4: TExprNode);
begin
  FFunc := AFunc;
  inherited Create([X1, X2, X3, X4]);
end;

procedure TExprCall4VmNodeOfObject.GenCode(AVirtMach: TExprVirtMach);
begin
  FExprVmCode := TExprCall4VmOpOfObject.Create(
    FFunc,
    VmDeps[0].ExprVmCode.OutputLoc,
    VmDeps[1].ExprVmCode.OutputLoc,
    VmDeps[2].ExprVmCode.OutputLoc,
...
{ TExpr1FuncSymOfObject }

function TExpr1FuncSymOfObject.Compile: TExprNode;
var
  X: TExprNode;
begin
  X := CompileFirstArg;
  EndArgs;
  Result := NodeFactory.Call1FuncOfObject(FFunc, X);
end;

constructor TExpr1FuncSymOfObject.Create(const AIdent: string; AFunc: T1FuncOfObject);
begin
  Assert(Assigned(AFunc));
  inherited Create(AIdent);
  FFunc := AFunc;
...
function TExpr1FuncSymOfObject.Evaluate: TFloat;
var
  X: TFloat;
begin
  X := EvalFirstArg;
...
{ TExpr2FuncSymOfObject }

function TExpr2FuncSymOfObject.Compile: TExprNode;
var
  X1, X2: TExprNode;
begin
  X1 := CompileFirstArg;
  X2 := CompileNextArg;
  EndArgs;
  Result := NodeFactory.Call2FuncOfObject(FFunc, X1, X2);
end;

constructor TExpr2FuncSymOfObject.Create(const AIdent: string; AFunc: T2FuncOfObject);
begin
  Assert(Assigned(AFunc));
  inherited Create(AIdent);
  FFunc := AFunc;
...
function TExpr2FuncSymOfObject.Evaluate: TFloat;
var
  X1, X2: TFloat;
begin
  X1 := EvalFirstArg;
...
{ TExpr3FuncSymOfObject }

function TExpr3FuncSymOfObject.Compile: TExprNode;
var
  X1, X2, X3: TExprNode;
begin
  X1 := CompileFirstArg;
  ...
  X3 := CompileNextArg;
  EndArgs;
  Result := NodeFactory.Call3FuncOfObject(FFunc, X1, X2, X3);
end;

constructor TExpr3FuncSymOfObject.Create(const AIdent: string; AFunc: T3FuncOfObject);
begin
  Assert(Assigned(AFunc));
  inherited Create(AIdent);
  FFunc := AFunc;
...
function TExpr3FuncSymOfObject.Evaluate: TFloat;
var
  X1, X2, X3: TFloat;
begin
  X1 := EvalFirstArg;
  ...
  X3 := EvalNextArg;
  EndArgs;
  Result := FFunc(X1, X2, X3);
end;

{ TExpr4FuncSymOfObject }

function TExpr4FuncSymOfObject.Compile: TExprNode;
var
  X1, X2, X3, X4: TExprNode;
begin
  X1 := CompileFirstArg;
  X2 := CompileNextArg;
  X3 := CompileNextArg;
  X4 := CompileNextArg;
  EndArgs;
  Result := NodeFactory.Call4FuncOfObject(FFunc, X1, X2, X3, X4);
end;

constructor TExpr4FuncSymOfObject.Create(const AIdent: string; AFunc: T4FuncOfObject);
begin
  Assert(Assigned(AFunc));
  inherited Create(AIdent);
  FFunc := AFunc;
...
function TExpr4FuncSymOfObject.Evaluate: TFloat;
var
  X1, X2, X3, X4: TFloat;
begin
  X1 := EvalFirstArg;
  ...
  X4 := EvalNextArg;
  EndArgs;
  Result := FFunc(X1, X2, X3, X4);
end;

{ TExprFuncSymOfObject }

function TExpr0FuncSymOfObject.Compile: TExprNode;
begin
  Result := NodeFactory.Call0FuncOfObject(FFunc);
end;

constructor TExpr0FuncSymOfObject.Create(const AIdent: string; AFunc: T0FuncOfObject);
begin
  Assert(Assigned(AFunc));
  inherited Create(AIdent);
  FFunc := AFunc;
...
function TExpr0FuncSymOfObject.Evaluate: TFloat;
begin
  Result := FFunc();
end;

{ TExprCall0VmNodeOfObject }

constructor TExprCall0VmNodeOfObject.Create(AFunc: T0FuncOfObject);
begin
  FFunc := AFunc;
  inherited Create([]);
end;

procedure TExprCall0VmNodeOfObject.GenCode(AVirtMach: TExprVirtMach);
begin
  FExprVmCode := TExprCall0VmOpOfObject.Create(
    FFunc
  );
  AVirtMach.Add(FExprVmCode);
end;

{ TExprCall0VmOpOfObject }

constructor TExprCall0VmOpOfObject.Create(AFunc: T0FuncOfObject);
begin
  inherited Create;
  FFunc := AFunc;
end;

procedure TExprCall0VmOpOfObject.Execute;
begin
  FOutput := FFunc();
end;
