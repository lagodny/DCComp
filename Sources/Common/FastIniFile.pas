...
{ TIniItems class }
  ...
  protected
    ...
    // Alex ++
    procedure AddTextStr(const Value: string);
    //
...
procedure TIniItems.AddTextStr(const Value: string);
var
  S: string;
  C, P, Start: PChar;
  aOldCapacity: Integer;
begin
  aOldCapacity := Capacity;
  Capacity := aOldCapacity + Length(Value) div 16;     // estimate 16 characters per item.
...
procedure TFastIniFile.AddStrings(aStrings: TStrings);
var
  i: Integer;
begin
  for i := 0 to aStrings.Count - 1 do
    FIniItems.Add(aStrings[i]);
end;

procedure TFastIniFile.AddText(aText: string);
begin
...
begin
  // Alex +-
  if FModified and (FileName <> '') then
//  if FModified then
  //
  begin
{$IFDEF COMPILER12_UP}
