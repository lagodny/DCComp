...
type
  ...
  public
    FRec1, FRec2: TXYS;
    function CalcColor(aRec: TXYS): TColor;
  ...
  published
    ...
    property OnAddXYS: TAddXYSProc read FOnAddXYS write FOnAddXYS;
  end;
...
procedure TDCSeriesAdapter.AssignTo(Dest: TPersistent);
var
  d: TDCSeriesAdapter;
begin
  if Dest is TDCSeriesAdapter then
  begin
    d := TDCSeriesAdapter(Dest);
//    d.DataLink := DataLink;
    d.PhysID := PhysID;
...
function TDCSeriesAdapter.CalcColor(aRec: TXYS): TColor;
begin
  if FRec1.s = 0 then
    Result := Series.Color
...
function TDCSeriesAdapter.CalcColor0: TColor;
begin
  if (FRec1.s = 0) then
    Result := Series.Color
  else
...
function TDCSeriesAdapter.CalcLabel(aValue: Double; aStateValue: extended): string;
var
...
procedure TDCSeriesAdapter.ChangeData(Sender: TObject);
var
  tmpY: double;
  Rec: TXYS;
  D2: TDateTime;
begin
  if not (Series.ParentChart is TDCChart) then
    Exit;

  // серія може мати власний обробник додавання даних
  ...
  // стандартна методіка додавання нових даних
  try
    ...
    on e: Exception do
      OPCLog.WriteToLog('Error in TDCSeriesAdapter.ChangeData : ' + e.Message);
  end;
end;

procedure TDCSeriesAdapter.ClearRecs;
begin
  FRec1.Clear;
  FRec2.Clear;
end;

constructor TDCSeriesAdapter.Create(aSeries: TChartSeries);
begin
  inherited Create;
  FSeries := aSeries;
...
destructor TDCSeriesAdapter.Destroy;
begin
...
procedure TDCSeriesAdapter.eAddXY(x, y: double);
...
procedure TDCSeriesAdapter.FillOPCData(ToNow, aSourceValues: Boolean);
var
...
procedure TDCSeriesAdapter.FillSerie(ToNow, aSourceValues: Boolean);
var
  Stream: TMemoryStream;
  aOPCSource: TaOPCSource;
...
function TDCSeriesAdapter.GetConnectionName: string;
begin
  Result := FConnectionName;
end;

function TDCSeriesAdapter.GetFilter: TaOPCFilter;
begin
  Result := FFilter;
end;

function TDCSeriesAdapter.GetFullName: string;
begin
  Result := FFullName;
end;

function TDCSeriesAdapter.GetInterval: TOPCInterval;
begin
  if Series.ParentChart is TDCChart then
    Result := TDCChart(Series.ParentChart).Interval
  else
    Result := nil;
end;

function TDCSeriesAdapter.GetOPCSource: TaCustomOPCSource;
begin
  Result := FOPCSource;
end;

function TDCSeriesAdapter.GetPhysID: TPhysID;
begin
  Result := FDataLink.PhysID;
end;

function TDCSeriesAdapter.GetRealTime: Boolean;
begin
  if Series.ParentChart is TDCChart then
    Result := TDCChart(Series.ParentChart).RealTime
  else
    Result := False;
end;

function TDCSeriesAdapter.GetScale: Double;
begin
  Result := FScale;
end;

function TDCSeriesAdapter.GetShift: Double;
begin
  Result := FShift;
end;

function TDCSeriesAdapter.GetShortName: string;
begin
  Result := FShortName;
end;

function TDCSeriesAdapter.GetShowFullName: Boolean;
begin
...
function TDCSeriesAdapter.RecIsActive(aRec: TXYS): Boolean;
begin
  Result := (aRec.x <> 0) and (aRec.y <> 0);
end;

procedure TDCSeriesAdapter.RestoreFromOriginal;
var
  i: Integer;
begin
  FSeries.Clear;
...
procedure TDCSeriesAdapter.sAddNullXY(x, y: double);
var
  aRec: TXYS;
begin
...
procedure TDCSeriesAdapter.sAddXY(aRec: TXYS);
...
//  end;
begin
  // якщо серія бажає використовувати свою процедуру додавання точок - викликаємо цю процедуру замість стандартної
  if Assigned(FOnAddXYS) then
  begin
    FOnAddXYS(aRec);
    Exit;
  end;
  ...
  // добавляем новое значение
  Series.AddXY(aRec.x, aRec.y / Scale + Shift, CalcLabel(aRec.y, aRec.s), CalcColor(aRec));
  FRec2 := FRec1;
  FRec1 := aRec;
end;

procedure TDCSeriesAdapter.sAddXY(x, y, s: double);
begin
  sAddXY(GetXYSRec(x, y, s));
end;

procedure TDCSeriesAdapter.SetConnectionName(const Value: string);
begin
  FConnectionName := Value;
end;

procedure TDCSeriesAdapter.SetDataLink(const Value: TaOPCDataLink);
begin
  FDataLink.Assign(Value);
end;

procedure TDCSeriesAdapter.SetDifferentialOrder(const Value: Integer);
begin
  FDifferentialOrder := Value;
end;

procedure TDCSeriesAdapter.SetDisplayFormat(const Value: string);
begin
  FDisplayFormat := Value;
end;

procedure TDCSeriesAdapter.SetFilter(const Value: TaOPCFilter);
begin
  FFilter.Assign(Value);
end;

procedure TDCSeriesAdapter.SetFullName(const Value: string);
begin
  FFullName := Value;
  UpdateTitle;
end;

procedure TDCSeriesAdapter.SetHideFiltered(const Value: Boolean);
begin
  FHideFiltered := Value;
end;

procedure TDCSeriesAdapter.SetIsState(const Value: boolean);
begin
  FIsState := Value;
end;

procedure TDCSeriesAdapter.SetLookupList(const Value: TaOPCLookupList);
begin
  FLookupList := Value;
end;

procedure TDCSeriesAdapter.SetOPCSource(const Value: TaCustomOPCSource);
begin
  FOPCSource := Value;
...
procedure TDCSeriesAdapter.SetPhysID(const Value: TPhysID);
begin
  FDataLink.PhysID := Value;
end;

procedure TDCSeriesAdapter.SetScale(const Value: double);
var
  i: integer;
begin
...
procedure TDCSeriesAdapter.SetShift(const Value: double);
var
  i: Integer;
begin
...
procedure TDCSeriesAdapter.SetShortName(const Value: string);
begin
...
procedure TDCSeriesAdapter.SetShowFullName(const Value: Boolean);
begin
...
procedure TDCSeriesAdapter.SetShowState(const Value: boolean);
begin
  FShowState := Value;
end;

procedure TDCSeriesAdapter.SetStairsOptions(const Value: TDCStairsOptionsSet);
begin
  FDataLink.StairsOptions := Value;
end;

procedure TDCSeriesAdapter.SetStateLookupList(const Value: TaOPCLookupList);
begin
  FStateLookupList := Value;
end;

procedure TDCSeriesAdapter.SetStatisticalFunc(const Value: TStatisticalFunc);
begin
  if FStatisticalFunc <> Value then
    FStatisticalFunc := Value;
end;

procedure TDCSeriesAdapter.SetStatisticalPeriod(const Value: TDateTime);
begin
  FStatisticalPeriod := Value;
end;

procedure TDCSeriesAdapter.StoreToOriginal;
var
  i: Integer;
begin
...
procedure TDCSeriesAdapter.UpdateRealTime;
begin
  if Assigned(Series.ParentChart) and (Series.ParentChart is TDCChart) then
  begin
    if TDCChart(Series.ParentChart).RealTime then
...
procedure TDCSeriesAdapter.UpdateRecs(aRec: TXYS);
begin
  FRec2 := FRec1;
  FRec1 := aRec;
end;

procedure TDCSeriesAdapter.UpdateTitle;
var
  tmpName: string;
  i: integer;
  aDifOrd: integer;
