...
//  with ProposedBoundsRect do Ctrl.SetBounds(left,top,right, bottom)
  with ProposedBoundsRect do Ctrl.SetBounds(left,top,Ctrl.Width, Ctrl.Height)
...
begin
  ...
  fFocusRect.Left := max(0,fFocusRect.Left);
  fFocusRect.Right := fFocusRect.Left + w;
  fFocusRect.Top := max(0,fFocusRect.Top);
...
begin
  ...
  fParentForm := GetParentForm(TWinControl(owner),False);
...
//------------------------------------------------------------------------------
type
  TCrackControl = class(TCustomControl)
  end;
...
begin
  ...
  begin
    ...
    // ++++
    if (targetIdx < 0) or (targetIdx >= fTargetList.Count) then
      exit;
    // ++++ \\
